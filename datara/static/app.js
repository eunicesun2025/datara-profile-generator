...
function render() {
  ...
  $('#main').innerHTML=`<button class="small ghost" data-action="home">← 返回我的 Profiles</button><div class="page-heading"><div><div class="eyebrow">PROFILE WORKSPACE</div><h1>${esc(p.name)}</h1><p class="subtitle">先编辑字段，再预览 SQL 和提示词；只有测试提取需要模型连接。</p></div>
    ...
    ${state.job?.status==='running'?`<div class="notice job-banner"><span><span class="spinner"></span> 正在${state.job.kind==='draft'?'分析文档并建议字段':'读取样本并提取数据'}…</span><button class="small" data-action="cancel-job">取消请求</button></div>`:''}
...
function samplePanel() {
  return `<section class="panel sample-panel"><div class="panel-head"><h3>样本单据</h3><button class="small ghost" data-action="upload">${state.sample?'更换':'上传'}</button></div><div class="panel-content">${state.sample ?
    ...
    `<div class="sample-drop" data-action="upload" role="button" tabindex="0"><div class="upload-icon">▤</div><strong>上传样本 PDF</strong><p>也支持 PNG / JPEG<br>最大 20MB · PDF 最多 15 页</p></div>`}
    <div class="sample-note">样本保存在本机。只有点击「AI 建议字段」或「测试提取」时，才会发送页面图片到配置的模型端点。</div></div></section>`;
}
function renderFields() {
  const p=state.profile,t=currentTable();
  $('#workspace-view').innerHTML=`<div class="editor-layout"><div class="stack">${samplePanel()}<div class="panel"><div class="panel-content"><div class="eyebrow">FIELD SOURCES</div><p class="helper"><span class="pill">AI · ${fieldCount('AI')}</span> 从单据提取，进入提示词</p><p class="helper"><span class="pill">Manual · ${fieldCount('Manual')}</span> 人工填写</p><p class="helper"><span class="pill">System · ${fieldCount('System')}</span> Datara 或数据库填充</p></div></div></div>
    ...
    <div class="field-toolbar"><div class="actions"><select id="source-filter" aria-label="筛选字段来源">${['all','AI','Manual','System'].map(v=>`<option value="${v}" ${state.filter===v?'selected':''}>${v==='all'?'全部字段':v}</option>`).join('')}</select><button class="small ghost" data-action="edit-table">修改${t.role==='head'?'主':'子'}表名称</button></div><div class="actions"><button class="small" data-action="import">导入 Excel</button><button class="small secondary" data-action="draft" ${state.job?.status==='running'?'disabled':''}>✧ AI 建议字段</button><button class="small primary" data-action="add-field">＋ 添加字段</button></div></div>
    <div class="field-scroll"><table class="field-table"><thead><tr><th></th><th>字段名称 / 说明</th><th>来源</th><th>类型</th><th>必填</th><th>操作</th></tr></thead><tbody>${t.fields.filter(f=>state.filter==='all'||f.source===state.filter).map(f=>fieldRow(f)).join('')}</tbody></table></div>
...
async function startJob(kind,instructions='') {
  ...
  if(kind==='extract')state.view='test';render();pollJob(state.job.id);
}
async function pollJob(id) {
  try {
    ...
    if(r.kind==='draft'&&r.status==='completed') {
      ...
      modal('审核 AI 建议字段',`<p class="helper" style="margin:0 0 15px">勾选要加入的字段。现有字段保持原样；未选择的建议不会进入输出。</p>${state.suggestions.length?state.suggestions.map((s,i)=>`<div class="suggestion"><label class="checkbox-label"><input type="checkbox" data-suggestion="${i}"><strong>${esc(s.field.name)}</strong><span class="pill">${esc(s.field.data_type)}</span></label><p>${esc(s.field.description)}<br>${esc(s.evidence)}</p></div>`).join(''):'<div class="notice">没有新的可添加字段。可以修改提取需求或手动添加。</div>'}`, '<button class="primary" data-action="apply-suggestions">添加所选字段</button>');
    }
...
const actions={
  ...
  draft:()=>{if(!state.sample)throw Error('请先上传样本');modal('AI 建议字段',`<div class="form-group"><label class="form-label" for="draft-request">希望提取哪些内容？</label><textarea id="draft-request" placeholder="例如：发票号码、日期、供应商名称、总金额，以及每行项目的说明与金额。"></textarea><p class="helper">AI 会结合当前表结构提出新增建议。要生成子表字段，请先添加对应子表。</p></div>`, '<button class="primary" data-action="run-draft">分析样本</button>');},
  ...
  'apply-suggestions':()=>{let n=0;for(const el of $$('[data-suggestion]:checked')){const s=state.suggestions[Number(el.dataset.suggestion)];const t=state.profile.tables.find(t=>t.id===s.table_id);if(t&&!t.fields.some(f=>f.name===s.field.name)){t.fields.push(s.field);n++;}}dirty();closeModal();state.view='fields';state.filter='all';render();toast(`已加入 ${n} 个字段，请在字段详情中审核并确认`);},
...
document.addEventListener('input',event=>{
  ...
  if(el.dataset.field&&el.tagName==='INPUT'){
    ...
    f[el.dataset.field]=el.type==='checkbox'?el.checked:el.value;dirty();
  }
});
...
$('#sample-upload').addEventListener('change',async event=>{
  const file=event.target.files[0];if(!file)return;const data=new FormData();data.append('file',file);toast('正在读取和准备样本页面…');
  try{const sample=await api('/samples',data);state.sample=sample;state.page=1;state.profile.sample_ids=[...new Set([...state.profile.sample_ids,sample.id])];dirty();render();toast('样本已载入，本地预览就绪');}catch(e){toast(e.message);}finally{event.target.value='';}
});
