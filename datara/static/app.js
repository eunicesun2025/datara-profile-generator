...
const state = {profile:null, profiles:[], meta:null, settings:null, view:'fields', tableId:null,
  ...
  optimizer:{cases:[],versions:[],activeVersionId:null,selected:new Set(),policies:{},run:null,iterations:[],
    maxIterations:3,minImprovement:0.01,maxRegressionDrop:0,noImprovementLimit:2,reuseBaseline:true}};
...
function renderOptimizer() {
  ...
  const caseCount=failureCount+regressionCount,estimatedMaxCalls=caseCount?caseCount*2+o.maxIterations*(caseCount+1):0;
  ...
  $('#workspace-view').innerHTML=`
    ...
    <div class="optimizer-bottom-grid">
      <section class="panel optimizer-panel optimizer-settings-panel"><div class="panel-head optimizer-panel-head"><div class="optimizer-section-title"><span>04</span><div><h3>设置优化停止条件</h3><p>决定最多尝试多久、什么结果可以保留；不会改变你选择的字段范围。</p></div></div></div><div class="panel-content"><div class="optimizer-settings-help"><strong>默认使用快速设置</strong><span>最多 3 轮、连续 2 轮没变好就停止；相同文档和提示词在 24 小时内会复用 Baseline，最终验证仍会真实重跑。</span></div><div class="optimizer-setting-grid"><label><span>最多尝试几轮</span><div class="optimizer-number-input"><input id="opt-max" type="number" min="1" max="20" value="${o.maxIterations}"><b>轮</b></div><small>每轮包含一次规则分析，并重新提取全部案例；达到目标会提前停止。</small></label><label><span>几轮没变好就停止</span><div class="optimizer-number-input"><input id="opt-plateau" type="number" min="1" max="10" value="${o.noImprovementLimit}"><b>轮</b></div><small>默认 2：连续 2 轮都没有更好结果就结束，避免无效等待。</small></label><label><span>新版本至少要提高</span><div class="optimizer-number-input"><input id="opt-min" type="number" min="0" max="100" step="0.5" value="${Number((o.minImprovement*100).toFixed(2))}"><b>%</b></div><small>低于这个提升不会被采用。1% 表示准确率至少提高 1 个百分点。</small></label><label><span>原本正确案例最多可下降</span><div class="optimizer-number-input"><input id="opt-regression" type="number" min="0" max="5" step="0.5" value="${Number((o.maxRegressionDrop*100).toFixed(2))}"><b>%</b></div><small>有保护案例时建议保持 0%，即新规则不能让原本正确的结果变差。</small></label><label><span>复用最近的 Baseline</span><div class="optimizer-number-input"><input id="opt-cache" type="checkbox" ${o.reuseBaseline?'checked':''}></div><small>相同端点、模型、提示词、结构和文档在 24 小时内不重复提取；取消勾选可强制重新测试。</small></label></div><div class="optimizer-run-summary ${ready&&!regressionCount?'is-warning':''}"><div><strong>${ready?(regressionCount?'可以开始优化':'可以运行，但缺少保护案例'):'现在还不能开始'}</strong><span>${ready?`当前设置预计最多 ${estimatedMaxCalls} 次模型调用；达到 100% 或连续 ${o.noImprovementLimit} 轮没提升会提前停止。`:!o.selected.size?'请先在上方勾选至少一个要优化的字段。':'请先添加至少一个待修复 PDF，并填写正确答案。'}</span></div><span class="optimizer-ready-dot ${ready?'is-ready':''}"></span></div><button class="primary optimizer-run-button" data-action="run-optimizer" ${running||!ready?'disabled':''}><span>${running?'正在优化所选字段…':'✦ 开始优化所选字段'}</span><small>自动尝试新规则，最终保留得分最高的版本</small></button></div></section>
