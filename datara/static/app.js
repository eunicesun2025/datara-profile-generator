...
const state = {profile:null, profiles:[], meta:null, settings:null, view:'fields', tableId:null,
  ...
  optimizer:{cases:[],versions:[],activeVersionId:null,selected:new Set(),policies:{},run:null,iterations:[],
    maxIterations:3,minImprovement:0.01,maxRegressionDrop:0,noImprovementLimit:2,reuseBaseline:true,evaluationRepeats:3}};
...
function toast(message,type='info') {
  const el=$('#toast');el.textContent=message||'发生未知错误';
  el.classList.toggle('toast-error',type==='error');el.classList.toggle('toast-warning',type==='warning');
  if(typeof el.showPopover==='function'){
    // Reinsert an already-open popover so it is newer than any dialog in the browser's
    // Top Layer. Otherwise a modal opened after the previous toast can cover a new error.
    if(el.matches(':popover-open'))el.hidePopover();
    el.showPopover();
  }
  el.classList.add('visible');clearTimeout(toastTimer);
  toastTimer=setTimeout(()=>{el.classList.remove('visible');if(typeof el.hidePopover==='function'&&el.matches(':popover-open'))el.hidePopover();},type==='error'?12000:6500);
}
...
async function loadProfile(p, view='fields', navigate=true) {
  ...
  state.optimizer={cases:[],versions:[],activeVersionId:null,selected:new Set(),policies:{},run:null,iterations:[],
    maxIterations:3,minImprovement:0.01,maxRegressionDrop:0,noImprovementLimit:2,reuseBaseline:true,evaluationRepeats:3};
  for(const id of p.reference_ids) {try{state.references.push(await api('/references/'+id));}catch(e){toast(e.message,'error');}}
  if(p.sample_ids?.length) { try {state.sample=await api('/samples/'+p.sample_ids.at(-1)); state.page=1;} catch(e){toast(e.message,'error');} }
...
function optimizerProgressText(run) {
  ...
  parts.push(phase);
  if(p.evaluation_repeats)parts.push(`重复 ${p.repetition}/${p.evaluation_repeats}`);
...
function renderTest() {
  ...
  const stale=r && (!state.preview || r.fingerprint!==state.preview.fingerprint || r.sample_id!==state.sample?.id || (r.prompt_hash && r.prompt_hash!==state.preview.prompt_hash));
  $('#workspace-view').innerHTML=`<div class="test-layout"><div class="stack">${samplePanel()}<section class="panel"><div class="panel-content"><h3>模型连接</h3><p class="helper">${esc(state.settings?.extraction_model?.trim()||state.settings?.model||'尚未配置')}<br>深度思考：${state.settings?.enable_thinking==null?'自动':state.settings.enable_thinking?'开启':'关闭'}<br>${state.settings?.has_key?'API Key 已配置':'请先填写模型地址和 API Key'}</p><button class="small w-full" data-action="settings" style="margin-top:13px">模型设置</button><button class="primary w-full" data-action="extract" style="margin-top:10px" ${state.job?.status==='running'?'disabled':''}>开始测试提取</button><p class="helper">使用当前活动提示词版本（含导入或已发布的优化提示词），与保存后的预览及导出一致。结果不会改写字段定义。</p></div></section></div>
...
async function loadOptimizer() {
  if(!state.profile)return;
  const [cases,versions,latestRun]=await Promise.all([
    api('/optimizer/profiles/'+state.profile.id+'/test-cases'),
    api('/profiles/'+state.profile.id+'/prompt-versions'),
    api('/optimizer/profiles/'+state.profile.id+'/runs/latest')
  ]);
  ...
  state.optimizer.run=latestRun;
  state.optimizer.iterations=latestRun?await api('/optimizer/runs/'+latestRun.id+'/iterations'):[];
  render();
  if(['queued','baselining','optimizing','validating'].includes(latestRun?.status))pollOptimizer(latestRun.id);
}
function optimizerCaseList(role) {
  ...
  return rows.length?rows.map(c=>`<article class="optimizer-case"><div class="optimizer-case-main"><span class="optimizer-case-icon">${role==='failure'?'!':'✓'}</span><div><strong>${esc(c.name)}</strong><p>${esc(c.sample_name)} · ${c.page_count} 页</p></div></div><div class="optimizer-case-meta"><span>已填写 ${c.ground_truth?.covered_paths?.length||0} 个正确答案${c.observed_output?' · 含已知错误输出':''}</span><div class="actions"><button class="small ghost" data-action="edit-ground-truth" data-id="${c.id}">编辑正确答案</button>${role==='failure'?`<button class="small ghost" data-action="edit-observed-output" data-id="${c.id}">编辑历史错误</button>`:''}<button class="small danger" data-action="disable-test-case" data-id="${c.id}">移除</button></div></div><details><summary>查看正确答案</summary><pre>${esc(json(c.ground_truth?.expected_output||{}))}</pre></details>${c.observed_output?`<details><summary>查看已知错误输出</summary><pre>${esc(json(c.observed_output))}</pre></details>`:''}</article>`).join(''):`<div class="optimizer-case-empty"><span>${role==='failure'?'＋':'◇'}</span><strong>尚未添加${label}</strong><p>${role==='failure'?'上传当前提取错误的 PDF，并填写这个字段的正确答案。':'上传当前提取正确的代表性 PDF，用来防止新规则破坏旧结果。'}</p></div>`;
}
...
function optimizerIterationHTML(iteration) {
  ...
  const statusLabel={accepted:'已接受',rejected:'已拒绝',failed:'失败',analyzing:'正在分析',evaluating:'正在评估'}[iteration.status]||esc(iteration.status);
  return `<details class="panel" ${iteration.accepted||iteration.status==='failed'?'open':''}><summary class="details-toggle">Iteration ${iteration.number} · ${statusLabel} ${metric?'· Failure '+score(metric.failure_selected):''}</summary><div class="panel-content">
    ${iteration.decision_reasons?.length?`<div class="notice ${iteration.status==='failed'?'error':iteration.accepted?'':'warning'}">${iteration.decision_reasons.map(esc).join('；')}</div>`:''}
    ${iteration.diffs?.some(d=>d.old_rule_source==='imported_prompt')?`<div class="notice"><strong>本轮实际使用了导入提示词全文。</strong><br>下方“旧规则”只显示 Profile 中单独维护的字段规则，不代表导入提示词里没有相关内容。<button class="small ghost" data-action="view-prompt-version" data-id="${iteration.base_version_id}">查看本轮完整原提示词</button></div>`:''}
    ${repeatMetricsHTML(metric)}${iteration.analyses?.map(a=>`<div class="suggestion"><strong>${esc(a.table_name)}.${esc(a.field_name)}</strong><span class="pill">${esc(a.error_type)}</span><p>${esc(a.root_cause)}</p><p class="helper">${esc(a.suggested_change)}</p></div>`).join('')||''}
    ${iteration.diffs?.map(d=>`<div class="suggestion"><strong>${esc(d.table_name)}.${esc(d.field_name)}</strong><p class="helper">${esc(d.reason)}<br>Failure ${score(d.failure_accuracy_before)} → ${score(d.failure_accuracy_after)} · Regression ${score(d.regression_accuracy_before)} → ${score(d.regression_accuracy_after)}</p><div class="form-grid"><div><span class="form-label">${d.old_rule_source==='imported_prompt'?'Profile 字段规则（实际基线为导入全文）':'旧规则'}</span><pre>${esc(d.old_rule||(d.old_rule_source==='imported_prompt'?'未在 Profile 字段中单独维护；请查看本轮完整原提示词。':'（空）'))}</pre></div><div><span class="form-label">新规则（追加到所选字段）</span><pre>${esc(d.new_rule)}</pre></div></div></div>`).join('')||''}
  </div></details>`;
}
function repeatMetricsHTML(metrics, label='候选实测') {
  if(!metrics?.case_results?.length)return '';
  return `<div class="notice"><strong>${esc(label)} · 每案例通过次数（${metrics.evaluation_repeats} 次测试）</strong><ul>${metrics.case_results.map(c=>`<li>${esc(c.name)}：${c.passed}/${c.total} 次通过</li>`).join('')}</ul><small>通过指本案例被评分字段全部正确；这是有限次数的测试结果，不是长期成功概率。</small></div>`;
}
function renderOptimizer() {
  ...
  const caseCount=failureCount+regressionCount,estimatedMaxCalls=caseCount?caseCount*o.evaluationRepeats*2+o.maxIterations*(caseCount*o.evaluationRepeats+1):0;
  const running=['queued','baselining','optimizing','validating'].includes(run?.status);
  const canPromote=Boolean(run?.promotion_eligible&&run.baseline_version_id===o.activeVersionId&&run.base_profile_revision===state.profile.revision);
  ...
  const promptLanguage=active?.prompt_language==='zh-CN'?'中文':active?.prompt_language==='en'?'英文':'混合/未识别';
  $('#workspace-view').innerHTML=`
    ...
    ${run?.status==='failed'&&run.blocking_reasons?.length?`<div class="notice error optimizer-top-error"><strong>本次优化失败</strong><br>${run.blocking_reasons.map(esc).join('；')}<br><small>候选提示词若已生成仍会保留在版本历史中。修改模型或超时设置后，需要重新开始一次优化才会生效。</small></div>`:''}
    <nav class="optimizer-lifecycle" aria-label="Profile lifecycle">
      ${[['01','生成','已完成'],['02','测试',failureCount?'已载入':'待准备'],['03','优化',running?'运行中':'当前阶段'],['04','验证',run?.final_validation_metrics?'已验证':'自动执行'],['05','发布',canPromote?'可发布':'人工确认']].map(([num,name,status],i)=>`<div class="lifecycle-step ${i<2?'is-done':''} ${i===2?'is-active':''} ${i>2?'is-next':''}"><span>${num}</span><div><strong>${name}</strong><small>${status}</small></div></div>`).join('<i>→</i>')}
    </nav>
    ...
    <section class="panel optimizer-panel optimizer-baseline-panel">
      <div class="panel-content optimizer-baseline-content"><div class="optimizer-baseline-icon">00</div><div class="optimizer-baseline-copy"><span>第一步：确认优化从哪一版提示词开始</span><h3>${imported?'正在使用你导入的已发布提示词':'当前使用系统根据 Profile 生成的提示词'}</h3><p>${imported?`基线测试使用提示词全文；已明确识别的字段规则会同步到 Profile，候选优化从这些旧规则继续。识别为${promptLanguage}。`:'如果线上已经有一套提示词，请先粘贴或上传 TXT；否则优化结果可能与线上版本不是同一起点。'}</p></div><div class="optimizer-baseline-actions"><button class="small ghost" data-action="view-prompt-version" data-id="${active?.id||''}" ${active?'':'disabled'}>查看当前提示词全文</button><button class="primary" data-action="import-existing-prompt">${imported?'替换已发布提示词':'导入已发布提示词'}</button></div></div>
    </section>
    <section class="panel optimizer-panel optimizer-fields-panel">
      ...
      <div class="panel-content optimizer-field-grid">${aiFields.map(f=>{const policy=o.policies[f.id]||((['Date','Integer','Decimal'].includes(f.data_type))?'normalized':'exact');const missingImportedRule=imported&&!f.extraction?.trim();const ruleLabel=imported?(missingImportedRule?'未同步到 Profile · 需要处理':'从已发布提示词同步的旧规则'):'当前字段规则';const ruleText=f.extraction||(imported?'没有识别到明确字段标题。重新导入规范提示词，或到字段详情补充规则后再优化。':'（空）');return `<article class="optimizer-field-card ${o.selected.has(f.id)?'is-selected':''} ${missingImportedRule?'has-rule-error':''}"><label class="optimizer-field-heading"><input type="checkbox" data-opt-field="${f.id}" ${o.selected.has(f.id)?'checked':''}><span><strong>${esc(f.name)}</strong><small>${esc(f.table_name)}</small></span><b>${esc(f.data_type)}</b></label><p class="optimizer-field-description">${esc(f.description||'未填写字段说明')}</p><div class="optimizer-rule-preview"><span>${ruleLabel}</span><p>${esc(ruleText)}</p></div><label class="optimizer-policy"><span>答案如何比较</span><select data-opt-policy="${f.id}"><option value="exact" ${policy==='exact'?'selected':''}>必须完全一致</option>${f.data_type==='String'?`<option value="case_insensitive" ${policy==='case_insensitive'?'selected':''}>忽略大小写与首尾空格</option>`:''}<option value="normalized" ${policy==='normalized'?'selected':''}>忽略日期/金额格式</option>${f.data_type==='String'?`<option value="semantic" ${policy==='semantic'?'selected':''}>文字含义相近即可</option>`:''}</select></label></article>`}).join('')}</div>
    </section>
    ...
    <div class="optimizer-bottom-grid">
      <section class="panel optimizer-panel optimizer-settings-panel"><div class="panel-head optimizer-panel-head"><div class="optimizer-section-title"><span>04</span><div><h3>设置优化停止条件</h3><p>决定最多尝试多久、什么结果可以保留；不会改变你选择的字段范围。</p></div></div></div><div class="panel-content"><div class="optimizer-settings-help"><strong>默认每个案例重复测试 3 次</strong><span>原版、每轮候选和最终验证使用相同重复次数。历史错误只供分析，不混入准确率；多次全对仍不代表长期稳定。</span></div><div class="optimizer-setting-grid"><label><span>每个案例重复测试</span><div class="optimizer-number-input"><input id="opt-repeats" type="number" min="1" max="10" value="${o.evaluationRepeats}"><b>次</b></div><small>默认 3 次，可设 1–10 次。超过 1 次时禁用基线缓存，耗时和费用随次数增加。</small></label><label><span>最多尝试几轮</span><div class="optimizer-number-input"><input id="opt-max" type="number" min="1" max="20" value="${o.maxIterations}"><b>轮</b></div><small>每轮包含一次规则分析，并重新提取全部案例；达到目标会提前停止。</small></label><label><span>几轮没变好就停止</span><div class="optimizer-number-input"><input id="opt-plateau" type="number" min="1" max="10" value="${o.noImprovementLimit}"><b>轮</b></div><small>默认 2：连续 2 轮都没有更好结果就结束，避免无效等待。</small></label><label><span>新版本至少要提高</span><div class="optimizer-number-input"><input id="opt-min" type="number" min="0" max="100" step="0.5" value="${Number((o.minImprovement*100).toFixed(2))}"><b>%</b></div><small>低于这个提升不会被采用。1% 表示准确率至少提高 1 个百分点。</small></label><label><span>原本正确案例最多可下降</span><div class="optimizer-number-input"><input id="opt-regression" type="number" min="0" max="5" step="0.5" value="${Number((o.maxRegressionDrop*100).toFixed(2))}"><b>%</b></div><small>有保护案例时建议保持 0%，即新规则不能让原本正确的结果变差。</small></label><label><span>复用最近的 Baseline</span><div class="optimizer-number-input"><input id="opt-cache" type="checkbox" ${o.evaluationRepeats===1&&o.reuseBaseline?'checked':''} ${o.evaluationRepeats>1?'disabled':''}></div><small>相同端点、模型、提示词、结构和文档在 24 小时内不重复提取；取消勾选可强制重新测试。</small></label></div><div class="optimizer-run-summary ${ready&&!regressionCount?'is-warning':''}"><div><strong>${ready?(regressionCount?'可以开始优化':'可以运行，但缺少保护案例'):'现在还不能开始'}</strong><span>${ready?`当前设置预计最多 ${estimatedMaxCalls} 次常规模型调用（不含网络重试及候选格式修复）；达到 100% 或连续 ${o.noImprovementLimit} 轮没提升会提前停止。`:!o.selected.size?'请先在上方勾选至少一个要优化的字段。':'请先添加至少一个待修复 PDF，并填写正确答案。'}</span></div><span class="optimizer-ready-dot ${ready?'is-ready':''}"></span></div><button class="primary optimizer-run-button" data-action="run-optimizer" ${running||!ready?'disabled':''}><span>${running?'正在优化所选字段…':'✦ 开始优化所选字段'}</span><small>自动尝试新规则，最终保留得分最高的版本</small></button></div></section>
    ...
    ${run?`<section class="panel optimizer-panel optimizer-result-panel"><div class="panel-head optimizer-panel-head"><div class="optimizer-section-title"><span>✓</span><div><h3>运行结果</h3><p>最终保留所有尝试中表现最好的一版，而不是直接使用最后一轮。</p></div></div><span class="result-status">${esc(run.status)}</span></div><div class="panel-content">${run.blocking_reasons?.length?`<div class="notice ${run.status==='failed'?'error':'warning'}">${run.blocking_reasons.map(esc).join('；')}</div>`:''}${repeatMetricsHTML(run.baseline_metrics, '原版基线')}${run.final_validation_metrics?repeatMetricsHTML(run.final_validation_metrics, '最终复验'):''}<div class="optimizer-result-stats"><div><small>优化前：待修复字段准确率</small><strong>${score(run.baseline_metrics?.failure_selected)}</strong></div><div><small>优化后：待修复字段准确率</small><strong>${score(run.best_metrics?.failure_selected)}</strong></div><div><small>保护案例整体准确率</small><strong>${score(run.final_validation_metrics?.regression_all||run.best_metrics?.regression_all)}</strong></div><div><small>表现最好的轮次</small><strong>${run.best_iteration_number||'—'}</strong></div></div><p class="helper">停止原因：${esc(run.stop_reason||'运行中')} · ${canPromote?'已通过最终验证，可以发布':run.promotion_eligible?'该结果对应较早的 Profile 或提示词，请重新运行':'尚未满足发布条件'}${run.cache_hits?` · 已复用 ${run.cache_hits} 个 Baseline 结果`:''}</p>${run.result_summary?`<div class="notice">修复 ${run.result_summary.fixed.length} 项 · 剩余失败 ${run.result_summary.remaining_failures.length} 项 · 新增回归 ${run.result_summary.regressed.length} 项</div>${run.result_summary.remaining_failures.map(x=>`<p class="helper">${esc(x.test_case_name)} · ${esc(x.path)} · 期望 ${esc(json(x.expected))} / 实际 ${esc(json(x.actual))}</p>`).join('')}`:''}${canPromote?'<button class="primary" data-action="promote-optimizer">发布为当前提示词</button>':''}</div></section><div class="optimizer-iterations">${o.iterations.map(optimizerIterationHTML).join('')}</div>`:''}`;
  installPanelControls();
}
...
async function settingsDialog() {
  ...
  modal('视觉模型设置',`<p class="helper" style="margin:0 0 20px">支持 OpenAI-compatible 视觉端点。模型 ID 请使用供应商实际提供的名称。</p>
    ...
    <div class="form-group"><label class="form-label" for="s-key">API Key</label><input id="s-key" type="password" autocomplete="off" placeholder="${s.has_key?'已配置；留空保留当前密钥':'输入 API Key'}"><p class="helper">页面填写的密钥只保留在本次服务运行中。若需重启后自动加载，请在项目根目录的 .env 中设置 DATARA_API_KEY；系统环境变量优先。</p></div>
    ...
    <div class="form-grid"><div class="form-group"><label class="form-label" for="s-optimizer-model">优化分析模型（可选）</label><input id="s-optimizer-model" value="${esc(s.optimizer_model||'')}" placeholder="留空使用默认模型"><p class="helper">建议使用支持图片的 Qwen 模型，以便结合失败页面判断字段区域；不支持图片时会退回文本分析。</p></div><div class="form-group"><label class="form-label" for="s-extraction-model">文档提取模型（可选）</label><input id="s-extraction-model" value="${esc(s.extraction_model||'')}" placeholder="留空使用默认模型"></div></div>
    <div class="form-group"><label class="form-label" for="s-thinking">深度思考</label><select id="s-thinking"><option value="auto" ${s.enable_thinking==null?'selected':''}>自动（兼容现有配置）</option><option value="on" ${s.enable_thinking===true?'selected':''}>开启</option><option value="off" ${s.enable_thinking===false?'selected':''}>关闭</option></select><p class="helper">同时用于 AI 分析、提示词优化和测试提取。开启时使用流式接收，可能增加耗时和 token 用量；模型及网关须支持 enable_thinking。自动模式仅对官方 Qwen 3.8 Max / Flash 关闭思考，其他模型使用供应商默认值。保存后对新任务生效。</p></div>
    <div class="form-grid"><div class="form-group"><label class="form-label" for="s-timeout">超时（秒）</label><input id="s-timeout" type="number" min="10" max="1800" value="${s.timeout}"><p class="helper">每次逻辑请求（含网络重试）的总上限；1800 表示最多等待 30 分钟。优化任务有多次请求，总时长可能更长。</p></div><div class="form-group"><label class="form-label" for="s-tokens">最大输出长度（tokens）</label><input id="s-tokens" type="number" min="256" max="32768" value="${s.max_tokens}"></div></div><div id="connection-test-result" class="helper" role="status"></div>`,
...
async function persistSettings() {
  state.settings=await api('/settings',{base_url:$('#s-url').value.trim(),model:$('#s-model').value.trim(),optimizer_model:$('#s-optimizer-model').value.trim(),extraction_model:$('#s-extraction-model').value.trim(),enable_thinking:$('#s-thinking').value==='auto'?null:$('#s-thinking').value==='on',api_key:$('#s-key').value||null,timeout:Number($('#s-timeout').value),max_tokens:Number($('#s-tokens').value)});
...
async function pollJob(id) {
  ...
  }catch(e){toast(e.message,'error');pollTimer=setTimeout(()=>pollJob(id),4000);}
}
async function pollOptimizer(id) {
  try {
    ...
    toast(run.status==='completed'?(run.promotion_eligible?'优化完成并通过最终验证':'优化完成，请查看结果'):'优化任务'+run.status,run.status==='failed'?'error':'info');
  }catch(e){toast(e.message,'error');pollTimer=setTimeout(()=>pollOptimizer(id),4000);}
}
...
const actions={
  ...
  'delete-profile':el=>{const p=state.profiles.find(x=>x.id===el.dataset.id);if(!p)throw Error('列表已过期，请返回「我的 Profiles」后重试');
    modal('删除 Profile',`<div class="notice error"><strong>此操作不可撤销。</strong><br>将永久删除「${esc(p.name)}」，包括它的提示词版本、优化记录、测试历史和导出文件。</div><p class="helper">当前保存版本 v${p.revision} · ${p.table_count} 张表 · ${p.field_count} 个字段<br>已上传的样张和参考文件会保留，可在其他 Profile 中继续使用。</p>`,'<button class="danger" data-action="confirm-delete-profile" data-id="'+esc(p.id)+'">确认永久删除</button>');},
  'confirm-delete-profile':async el=>{const r=await api('/profiles/'+el.dataset.id,undefined,{method:'DELETE'});closeModal();
    if(state.profile?.id===r.id){clearTimeout(pollTimer);state.job=null;state.profile=null;state.dirty=false;}
    state.profiles=await api('/profiles');pushRoute();render();
    const removed=r.removed||{},labels=[['optimizer/prompt_versions','提示词版本'],['optimizer/runs','优化任务'],['optimizer/test_cases','优化案例'],['tests','测试记录'],['exports','导出文件']];
    const cleaned=labels.filter(([k])=>removed[k]).map(([k,label])=>`${label} ${removed[k]}`).join(' · ');
    toast(`已删除「${r.name}」${cleaned?`，同时清理 ${cleaned}`:''}`);},
  ...
  'test-connection':async()=>{await persistSettings();$('#connection-test-result').textContent='正在测试连接…';try{const r=await api('/settings/test',{});$('#connection-test-result').textContent=r.message;}catch(e){$('#connection-test-result').textContent=e.message;toast(e.message,'error');}},
  ...
  'save-imported-prompt':async()=>{
    ...
    const result=await api('/profiles/'+state.profile.id+'/prompt-versions/import',{expected_profile_revision:state.profile.revision,prompt_text:text});
    state.profile=result.profile;state.dirty=false;closeModal();await loadOptimizer();
    const mapped=result.mapped_field_ids?.length||0,unmapped=result.unmapped_field_ids?.length||0,rejected=result.rejected_field_ids?.length||0;
    toast(`已发布提示词已设为基线；同步 ${mapped} 个字段规则${unmapped?`，${unmapped} 个字段未同步${rejected?`（其中 ${rejected} 个与当前规范冲突）`:''}`:''}`,unmapped?'warning':'info');
  },
  ...
  'create-optimizer-case':async()=>{
    ...
    let observed=null;const observedText=$('#optimizer-observed-output')?.value.trim();
    if(observedText){try{observed=JSON.parse(observedText);}catch{throw Error('已知错误输出必须是有效 JSON');}}
    await api('/optimizer/profiles/'+state.profile.id+'/test-cases',{sample_id:sample.id,name:$('#optimizer-case-name').value.trim()||sample.name,dataset_role:role,ground_truth:truth,observed_output:observed});
  ...
  'edit-observed-output':el=>{const c=state.optimizer.cases.find(c=>c.id===el.dataset.id);state.optimizer.editCaseId=c.id;modal('编辑历史错误 · '+c.name,`<p class="helper">粘贴该样本在当前原提示词下真实出现过的错误 JSON，只需相关字段。仅用于分析，不计入本次准确率；留空清除。保存后对新任务生效。</p><textarea id="optimizer-observed-edit" class="w-full" style="min-height:260px;font-family:monospace">${esc(c.observed_output?json(c.observed_output):'')}</textarea>`,'<button class="primary" data-action="save-observed-output">保存历史错误</button>');},
  'save-observed-output':async()=>{const text=$('#optimizer-observed-edit').value.trim();let observed=null;if(text){try{observed=JSON.parse(text);}catch{throw Error('历史错误必须是有效 JSON');}if(!observed||Array.isArray(observed)||typeof observed!=='object')throw Error('历史错误必须是 JSON 对象');}await api('/optimizer/test-cases/'+state.optimizer.editCaseId,{observed_output:observed},{method:'PATCH'});closeModal();await loadOptimizer();toast('历史错误已保存，下次优化生效');},
  ...
  'run-optimizer':async()=>{
    ...
    const activeVersion=state.optimizer.versions.find(v=>v.id===active);
    if(activeVersion?.prompt_source==='imported'){
      const missing=state.profile.tables.flatMap(t=>t.fields.filter(f=>selected.includes(f.id)&&!f.extraction?.trim()).map(f=>`${t.name}.${f.name}`));
      if(missing.length)throw Error('以下字段没有从已发布提示词同步到旧规则：'+missing.join('、')+'。请重新导入规范提示词，或先在字段详情中补充提取规则。');
    }
  ...
  'view-prompt-version':async el=>{const v=await api('/prompt-versions/'+el.dataset.id);state.optimizer.viewVersionId=v.id;modal('Prompt v'+v.version_number,`<p class="helper">${esc(v.origin)} · ${esc(v.lifecycle)} · Profile r${v.profile_revision}</p><pre style="max-height:55vh">${esc(v.rendered_prompt)}</pre>`,v.id===state.optimizer.activeVersionId||!['active','superseded'].includes(v.lifecycle)?'':'<button class="danger" data-action="rollback-prompt-version">回滚到此规则版本</button>');},
...
document.addEventListener('click',async event=>{
  ...
  try{await action(el);}catch(e){toast(e.message,'error');}finally{if(!hadDisabled)el.disabled=false;}
});
...
document.addEventListener('change',event=>{
  ...
  if(el.id==='opt-repeats'){state.optimizer.evaluationRepeats=Math.max(1,Math.min(10,Number(el.value)||3));renderOptimizer();return;}
  if(el.id==='opt-cache'){state.optimizer.reuseBaseline=el.checked;return;}
  if(el.id==='opt-min'){state.optimizer.minImprovement=Number(el.value)/100;return;}
  if(el.id==='opt-regression'){state.optimizer.maxRegressionDrop=Number(el.value)/100;return;}
  if(el.id==='opt-max'){state.optimizer.maxIterations=Number(el.value);renderOptimizer();return;}
  if(el.id==='opt-plateau'){state.optimizer.noImprovementLimit=Number(el.value);renderOptimizer();return;}
...
$('#sample-upload').addEventListener('change',async event=>{
  ...
  try{await uploadSampleFile(file);}catch(e){toast(e.message,'error');}finally{event.target.value='';}
});
$('#reference-upload').addEventListener('change',async event=>{try{await uploadReferences([...event.target.files]);}catch(e){toast(e.message,'error');}finally{event.target.value='';}});
...
document.addEventListener('drop',async event=>{if(!event.dataTransfer?.files.length)return;event.preventDefault();$$('.drag-active').forEach(el=>el.classList.remove('drag-active'));const zone=event.target.closest('[data-drop]');if(!zone)return;try{const files=[...event.dataTransfer.files];if(zone.dataset.drop==='reference')await uploadReferences(files);else {if(files.length!==1)throw Error('每次请拖入一份样张；多页请合并为 PDF');await uploadSampleFile(files[0]);}}catch(e){toast(e.message,'error');}});
document.addEventListener('focusout',async event=>{if(event.target.dataset.field!=='name')return;const snapshot=json(state.profile);try{const r=await api('/normalize',state.profile);if(!state.profile||json(state.profile)!==snapshot)return;if(r.notes.length){state.profile=r.profile;dirty();render();toast(r.notes.join('；'));}}catch(e){toast(e.message,'error');}});
...
$('#excel-upload').addEventListener('change',async event=>{
  const file=event.target.files[0];if(!file)return;const data=new FormData();data.append('file',file);
  try{state.importData=await api('/import/inspect',data);state.importData.filename=file.name;importDialog();}catch(e){toast(e.message,'error');}finally{event.target.value='';}
});
window.addEventListener('beforeunload',event=>{if(state.dirty){event.preventDefault();event.returnValue='';}});
window.addEventListener('popstate',async()=>{try{if(state.dirty)await save();await restoreRoute();}catch(e){pushRoute();toast(e.message,'error');}});
(async()=>{try{[state.meta,state.settings,state.profiles]=await Promise.all([api('/meta'),api('/settings'),api('/profiles')]);if(!location.hash)window.history.replaceState(null,'','#home');await restoreRoute();}catch(e){$('#main').innerHTML=`<div class="notice error">页面未能打开：${esc(e.message)} <button data-action="home">返回我的 Profiles</button></div>`;toast(e.message,'error');}})();
