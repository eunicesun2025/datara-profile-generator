...
13. **Jobs are not durable work.** Records persist, but task execution and cancellation handles are memory-only. Restart converts `running` to `interrupted`; there is no retry or resume.
