...
3. 针对本次文件，第 38 行 `validity_check` 暂保留原表名并改为主表层级；业务人员仍需确认字段是否确实属于主表。明细缺失的父表补为唯一主表。
...
当前回归测试共 37 项，包含结构修复、表名修改后的主外键与输出一致性、保存重开、模型错误信息不泄露密钥等。模拟接口测试不能代替真实模型或 Datara 导入验收。
