...
- 公司代码、系统查询值、计算值、自动生成行号等由 Datara 填充，统一归为 `System`。
...
- 不自动补 `0`、空字符串、当前日期、示例值、默认货币或自动行号。
...
| 公司代码、自动 item 行号标为 AI | 按确认规则改为 System，并记录由 Datara 填充 |
...
旧文件用于验证兼容性和识别业务规则，不能作为新输出逐字一致的目标。现有预期 JSON 若包含自动行号、默认税额或默认数量，应先依据新规则形成审核后的测试基准，保留原文件不覆盖。
