...
- 在「我的 Profiles」列表卡片上删除不再需要的 Profile：级联清理它的提示词版本、优化记录、测试历史和导出文件；仍有模型或优化任务运行时会被拒绝。样张与参考文件属于共享上传，会保留下来。
...
- 优化界面默认每个案例重复测试 3 次，最多 3 轮/连续 2 轮无提升即停止；多次测试禁用 Baseline 缓存。原版、候选、最终验证使用相同重复次数，展示每个案例通过几次；单次模式可复用 24 小时内匹配的 Baseline，最终验证始终真实重跑。
...
修改数据目录请通过 `DATARA_DATA_DIR`。界面上删除 Profile 会同时清理它独有的提示词版本、优化记录、测试历史和导出文件；样张、参考文件和导入的 Excel 属于共享上传，不会被删除。本版本不自动清理这些上传文件与孤立样本，可在停止服务后备份或删除对应数据目录。多用户和多副本部署前需加入认证与共享存储；当前使用单进程本地文件存储。
...
- 系统查询、公司代码映射等 System 字段由 Datara 填充；发票明细 item 按用户确认配置为 AI/String，由模型按数组顺序返回行号。
...
- `observed_output` 只用于分析真实历史错误，不替代 Baseline，也不计入本次准确率。已有案例可点击「编辑历史错误」补充或清除，已启动任务的快照不变。
- `OptimizationSettings.evaluation_repeats` 支持 1–10 次；界面默认 3，API 省略时为 1 以兼容既有调用。大于 1 时不复用缓存，每次都调用模型；原版和候选都全对时显示尚未证明更好，候选保留但不可发布。重复测试提高发现偶发错误的机会，不是统计显著性证明，增加次数会增加耗时和费用。
