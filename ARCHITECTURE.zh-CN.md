...
5. **保存**：进程内锁保护乐观 revision 检查；保存增加 revision、写 UTC 时间并原子替换 JSON。保存本身不要求 Profile 有效。保存后 `ensure_prompt_version()` 只用提示词相关投影（`prompt_fingerprint()`：表名、字段顺序、提示词标题、字段属性与 AI 提取规则）判断活动基线是否过期，因此挂样张、选参考文件或改库名等工作区状态不会重建提示词版本；提示词相关内容确实变化时，导入基线会以 `imported_prompt_refresh` 新版本原样保留正文并只把变化的 AI 规则重新追加为覆盖块（生成器无法重建已发布的外部提示词），只有生成基线才会按 Profile 重新渲染。
