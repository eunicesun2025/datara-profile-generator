...
7. **AI 分析**：把完整当前字段/规则、一个样张的 JPEG 页面、选中的参考文本和用户字段需求发送到模型。上传内容明确被视为不可信数据。模型可建议单据类型、规则及现有表中的 AI 字段新增/更新，但不能修改 Manual/System 或创建表；用户勾选后才应用。
...
21. 参考文件会丢失部分格式；DOCX/XML 与 XLSX 只转换为文字/行，权威性由用户和模型判断。
